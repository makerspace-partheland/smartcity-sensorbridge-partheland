...
async def async_setup_entry(
    ...
    try:
        ...
        for device_id in selected_devices:
            device_entities = await create_device_entities(coordinator, device_id, config_service)
            entities.extend(device_entities)
        
        # Median-Entities
        selected_median_entities = entry.data.get("selected_median_entities", [])
        for median_id in selected_median_entities:
            median_entities = await create_median_entities(coordinator, median_id, config_service)
...
async def create_device_entities(
    ...
    try:
        ...
        for sensor_name in sensors:
            ...
            entity = SensorBridgeSensor(coordinator, entity_data, config_service)
            entities.append(entity)
        
        return entities
        
    except Exception as e:
        _LOGGER.error("Error creating device entities for %s: %s", device_id, e)
        return entities
...
async def create_median_entities(
    ...
    try:
        ...
        _LOGGER.error("Error creating median entities for %s: %s", median_id, e)
        return entities
...
class SensorBridgeSensor(SensorEntity):
    ...
    def __init__(
        ...
        # has_entity_name auf True setzen für Translation-Support
        self._attr_has_entity_name = True
        
        # Name wird durch Translation automatisch gesetzt, aber wir setzen ihn hier zur Sicherheit
        ...
        self._attr_device_info = DeviceInfo(
            ...
            name=device_name,
            manufacturer="SmartCity SensorBridge Partheland",
            model=device_id,
        )
        ...
        # Translation-Test asynchron ausführen
        self.hass = None  # Wird in async_added_to_hass gesetzt
        self._translation_test_pending = True
    ...
    async def async_added_to_hass(self) -> None:
        ...
        # Translation-Test ausführen wenn noch nicht gemacht
        if hasattr(self, '_translation_test_pending') and self._translation_test_pending:
            await self._test_translation_on_add()
            self._translation_test_pending = False
        ...
        _LOGGER.debug("SensorBridge sensor added to hass: %s", self._attr_unique_id)
    ...
    async def _load_sensor_attributes(self) -> None:
        ...
        try:
            ...
            # Translation laden und Namen setzen
            ...
            # Korrekte Struktur basierend auf de.json: entity.sensor.{sensor_name}.name
            ...
            if sensor_name in sensor_translations:
                ...
                if isinstance(sensor_data, dict) and "name" in sensor_data:
                    ...
                    _LOGGER.info("Translation found for %s: %s", sensor_name, self._attr_name)
                else:
                    _LOGGER.warning("Invalid translation structure for %s: %s", sensor_name, sensor_data)
            else:
                _LOGGER.warning("No translation found for sensor: %s", sensor_name)
                _LOGGER.debug("Available sensor translations: %s", list(sensor_translations.keys()))
            ...
            if device_class_str:
                self._attr_device_class = await self._get_device_class(device_class_str)
            
            # Icon setzen
            self._attr_icon = await self._get_sensor_icon(sensor_name, device_class_str)
            ...
            _LOGGER.info("Sensor attributes loaded for %s: name='%s', translation_key='%s', unit='%s', device_class='%s', icon='%s'", 
                         self._attr_unique_id, self._attr_name, self._attr_translation_key, unit, device_class_str, self._attr_icon)
            
        except Exception as e:
            _LOGGER.error("Error loading sensor attributes: %s", e)
    
    async def _get_device_class(self, device_class_str: str) -> Optional[SensorDeviceClass]:
        """Konvertiert String zu SensorDeviceClass."""
        try:
            # Device Class Mapping aus der Konfiguration laden
            device_class_mapping = await self.config_service.get_device_class_mapping()
            ...
            _LOGGER.error("Error converting device class %s: %s", device_class_str, e)
            return None
    
    async def _get_sensor_icon(self, sensor_name: str, device_class_str: Optional[str]) -> str:
        ...
        try:
            ...
            sensor_categories = await self.config_service.get_sensor_categories()
    ...
    async def async_will_remove_from_hass(self) -> None:
        ...
        _LOGGER.debug("SensorBridge sensor removed from hass: %s", self._attr_unique_id)
    ...
    async def test_translation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.info("Translation Test für %s: %s", self._attr_unique_id, test_result)
            return test_result
    ...
    async def force_name_update(self) -> None:
        ...
        try:
            ...
            _LOGGER.info("Entity-Name Update erzwungen für %s", self._attr_unique_id)
        except Exception as e:
    ...
    async def _test_translation_on_add(self) -> None:
        ...
        try:
            ...
            _LOGGER.info("=== TRANSLATION TEST für %s ===", self._attr_unique_id)
            ...
            if sensor_name in sensor_translations:
                sensor_data = sensor_translations[sensor_name]
                if isinstance(sensor_data, dict) and "name" in sensor_data:
                    _LOGGER.info("✅ Translation gefunden für %s: %s", sensor_name, sensor_data["name"])
                else:
                    _LOGGER.warning("❌ Invalid translation structure for %s: %s", sensor_name, sensor_data)
            else:
                _LOGGER.warning("❌ KEINE Translation gefunden für Key: %s", sensor_name)
                _LOGGER.warning("Verfügbare Keys: %s", list(sensor_translations.keys()))
